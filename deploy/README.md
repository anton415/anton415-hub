...
- `docker-compose.production.yml` runs PostgreSQL, migrations, the app image, production n8n, and Caddy on the VM.
...
Deployments extract `/app/migrations` and `/app/deploy/n8n/workflows` from the app image, run `migrate/migrate` on the VM, import/publish the Orchestrator n8n workflow, then restart the app, n8n, and Caddy.
