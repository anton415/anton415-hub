...
- n8n runs in Docker on the same VM for Orchestrator AI/GitHub workflow execution, backed by a separate PostgreSQL 16 container and persistent Docker volumes
...
6. The VM creates or updates `/opt/anton415-hub`, writes the current Compose/Caddy config, pulls the image, extracts migrations from it, runs migrations against the local PostgreSQL container, restarts the app/Caddy services, and checks `/health`.
   It also extracts the versioned Orchestrator n8n workflow, imports it into production n8n, marks it active, publishes it, and restarts n8n so the internal webhook URLs are live.
7. Check:
