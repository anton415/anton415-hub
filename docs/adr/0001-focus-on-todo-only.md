...
1. **No new feature work, refactors, non-blocking bug fixes, or documentation effort** is spent on Finance, Calendar, Orchestrator, Investments, FIRE, News, or AI chat. Non-blocking bugs are ignored until the module is removed. **Exception:** because frozen-module code is still served in production through Phases 0–6, a fix is allowed when (and only when) the issue is production-breaking (the app fails to start, an unaffected page becomes unreachable, data is corrupted or lost) or is a real security vulnerability. Such a fix must be the minimum change required to restore service or close the vulnerability — no surrounding cleanup, no refactor, no feature work — and the PR description must call out the exception and link to this ADR. This matches `AI.md` §4 ("Do not extend them, fix non-blocking bugs in them, or restore them").
